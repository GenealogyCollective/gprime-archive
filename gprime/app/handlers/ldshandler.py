...
from ..forms import LDSForm

class LDSHandler(BaseHandler):
    ...
    def get(self, prefix="", suffix=""):
        ...
        path = "lds_ord_list.%s" % (int(row) - 1)
        if prefix.count("/") > 2:
            parts = prefix.split("/") # "/person/handle/somethings/", ["", "person", "handle", "sometings"]
            path = ".".join(parts[3:]) + path
        url = "/" + prefix + "/lds_ord_list/" + row
        subitem = instance.get_field(path)
        form = LDSForm(self, instance, subitem, path, url)
        ## FIXME: Handle add and delete
        self.render("lds.html",
                    **self.get_template_dict(tview=_("lds"),
                                             action=action,
                                             search="",
                                             page="",
                                             form=form))
