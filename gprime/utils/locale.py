...
_icu_err = None
_hdlr = None
# Locale initialization comes before command-line argument
...
try:
    from icu import Locale as ICULocale, Collator
    HAVE_ICU = True
except ImportError:
    try:
        from PyICU import Locale as ICULocale, Collator
        HAVE_ICU = True
...
ICU_LOCALES = None
if HAVE_ICU:
    ICU_LOCALES = ICULocale.getAvailableLocales()
...
class Locale:
    ...
    new Locale instance with the specified parameters, but any
    ...
    def __new__(cls, localedir=None, lang=None, domain=None, languages=None):
        if not Locale.__first_instance:
            cls.__first_instance = super(Locale, cls).__new__(cls)
            cls.__first_instance.initialized = False
            return cls.__first_instance

        if not cls.__first_instance.initialized:
            raise RuntimeError("Second Locale created before first one was initialized")
        ...
        return super(Locale, cls).__new__(cls)
...
#localize their output, e.g. time.strftime(). NB: encoding is a class variable.
    ...
    def __init__(self, localedir=None, lang=None, domain=None, languages=None):
        """
        Init a Locale. Run __init_first_instance() to set up the
        ...
        if self == _first:
            self._Locale__init_first_instance()
        else:
        ...
        self.collator = None
        if HAVE_ICU:
            self.icu_locales["default"] = ICULocale.createFromName(self.lang)
            if self.collation and self.collation != self.lang:
                self.icu_locales["collation"] = ICULocale.createFromName(self.collation)
            else:
